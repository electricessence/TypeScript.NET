...
private bool; IsIntranet(string; schemeHost)
	{
...
#else
					s_ManagerRef = (IInternetSecurityManager);
		...
		try{
			s_ManagerRef.MapUrlToZone(schemeHost.TrimStart(_WSchars), out;
			zone, 0;
		)
		}
		catch(COMException
...
private void CreateUri(Uri; baseUri, string; relativeUri, bool; dontEscape)
	{
		...
		if (baseUri.Syntax.IsSimple)
		{
			...
			Uri;
			uriResult = ResolveHelper(baseUri, this, ref;
			relativeUri, ref;
			dontEscape, out;
			e;
		)
		...
		{
			dontEscape = false;
			relativeUri = baseUri.Syntax.InternalResolve(baseUri, this, out;
			e;
		)
			if (e != null)
				throw e;
...
public Uri(Uri; baseUri, Uri; relativeUri)
	{
		...
		if (baseUri.Syntax.IsSimple)
		{
			...
			relativeUri = ResolveHelper(baseUri, this, ref;
			newUriString, ref;
			dontEscape, out;
			e;
		)
		...
		{
			dontEscape = false;
			newUriString = baseUri.Syntax.InternalResolve(baseUri, this, out;
			e;
		)
			if (e != null)
				throw e;
...
private string; GetLocalPath(){
		...
		if (IsUncOrDosPath)
		{
			...
			if (InFact(Flags.ShouldBeCompressed)) {
				...
				result = Compress(result, (ushort)(IsDosPath ? pathStart + 2 : pathStart), ref;
				count, m_Syntax;
			)
			}
...
public static string; HexEscape(char; character) {
		...
		int; pos = 0;
		UriHelper.EscapeAsciiChar(character, chars, ref;
		pos;
	)
...
public override; int; GetHashCode(); {
	...
	//
	private const UriFormat;
	V1ToStringUnescape = (UriFormat);
	0x7FFF;
	[SecurityPermission(SecurityAction.InheritanceDemand, Flags=SecurityPermissionFlag.Infrastructure)];
...
public override; bool; Equals(object; comparand) {
		...
		if((object)this.m_String==(object)
		obj.m_String;
		;
	)
		{
...
private static bool; CheckForColonInFirstPathSegment(String; uriString)
	...
	internal; unsafe; static string; InternalEscapeString(string; rawString) {
		...
		int; position = 0;
		char[];
		dest = UriHelper.EscapeString(rawString, 0, rawString.Length, null, ref;
		position, true, '?', '#', '%';
	)
		if ((object)dest == null)
		return rawString;
...
private static unsafe; ParsingError; ParseScheme(string; uriString, ref; Flags; flags, ref; UriParser; syntax)
	{
		...
		fixed (char* pUriString = uriString);
		{
			...
			ushort;
			idx = ParseSchemeCheckImplicitFile(pUriString, (ushort);
			length, ref;
			err, ref;
			flags, ref;
			syntax;
...
private unsafe; ParsingError; PrivateParseMinimal();
	{
		...
		// Fro V1 compat the next block allow this case but not for schemes like http
			...
			{
				ParsingError; err = ParsingError.None;
				idx = CheckAuthorityHelper(pUriString, idx, (ushort);
				length, ref;
				err, ref;
				m_Flags, m_Syntax, ref;
				newHost;
			)
				if (err != ParsingError.None)
					return err;
...
private unsafe; void CreateHostString(); {
		...
		Flags; flags = m_Flags;
		string;
		host = CreateHostStringHelper(m_String, m_Info.Offset.Host, m_Info.Offset.Path, ref;
		flags, ref;
		m_Info.ScopeId;
	)
		...
		if (host.Length != 0)
		{
			...
			if (HostType == Flags.BasicHostType) {
				...
				fixed (char* pHost = host); {
					result = CheckCanonical(pHost, ref;
					idx, (ushort);
					host.Length, c_DummyChar;
				)
				}
...
private static string; CreateHostStringHelper(string; str, ushort; idx, ushort; end, ref; Flags; flags, ref; string; scopeId)
	{
		...
		switch (flags & Flags.HostTypeMask) {
			...
			case Flags.DnsHostType:
				host = DomainNameHelper.ParseCanonicalName(str, idx, end, ref;
				loopback;
			)
				break;
			...
			case Flags.IPv6HostType:
				...
				host = IPv6AddressHelper.ParseCanonicalName(str, idx, ref;
				loopback, ref;
				scopeId;
			)
				break;
			...
			case Flags.IPv4HostType:
				host = IPv4AddressHelper.ParseCanonicalName(str, idx, end, ref;
				loopback;
			)
				break;
				...
				host = UncNameHelper.ParseCanonicalName(str, idx, end, ref;
				loopback;
			)
				break;
#endif // !PLATFORM_UNIX
...
private unsafe; void GetHostViaCustomSyntax();
	{
		...
		if ((object)m_Info.Host == null)
		{
			...
			{
				host = CreateHostStringHelper(host, 0, (ushort);
				host.Length, ref;
				flags, ref;
				m_Info.ScopeId;
			)
...
private string; GetEscapedParts(UriComponents; uriParts) {
		...
		//
		ushort;
		nonCanonical = (ushort)(((ushort);
		m_Flags & ((ushort);
		Flags.CannotDisplayCanonical<<7;
	)) >>
		6;
	)
...
private string; ReCreateParts(UriComponents; parts, ushort; nonCanonical, UriFormat; formatAs)
	{
		...
		{
			chars = GetCanonicalPath(chars, ref;
			count, formatAs;
		)
					...
					case UriFormat.UriEscaped:
						if (NotAny(Flags.UserEscaped))
							chars = UriHelper.EscapeString(m_String, delimiterAwareIndex, m_Info.Offset.End, chars,
								ref;
						count, true, UriParser.ShouldUseLegacyV2Quirks
							? '#'
...
private unsafe; void ParseRemaining(); {
		...
		if (buildIriStringFromPath){
			...
			if (origIdx < m_originalUnicodeString.Length && m_originalUnicodeString[origIdx] == '?'){
				...
				FindEndOfComponent(m_originalUnicodeString, ref;
				origIdx, (ushort);
				m_originalUnicodeString.Length, ((syntaxFlags & (UriSyntaxFlags.MayHaveFragment))!=0)
					? '#'
					: c_EOL;
			)
		...
		if (buildIriStringFromPath){
			...
			if (origIdx < m_originalUnicodeString.Length && m_originalUnicodeString[origIdx] == '#')
			{
				...
				FindEndOfComponent(m_originalUnicodeString, ref;
				origIdx, (ushort);
				m_originalUnicodeString.Length, c_EOL;
			)
	...
	unsafe; static private ushort; ParseSchemeCheckImplicitFile(char *uriString, ushort; length,
		...
		err = CheckSchemeSyntax(schemePtr, length, ref;
		syntax;
	)
		if (err != ParsingError.None) {
			return 0;
	...
	unsafe; static private bool; CheckKnownSchemes(long *lptr, ushort; nChars, ref; UriParser; syntax) {
		...
		const long;
		_LOWERCASE_Mask = 0x0020002000200020;
		L;
		...
		case if(nChars==6 && (*(int*)(lptr + 1) | _INT_LOWERCASE_Mask) == _GOPHER_Mask1) {
				syntax = UriParser.GopherUri;
				return true;
			}
			break;
		case if(nChars==6 && (*(int*)(lptr + 1) | _INT_LOWERCASE_Mask) == _MAILTO_Mask1) {
				syntax = UriParser.MailToUri;
				return true;
			}
			break;

		case if(nChars==6 && (*(int*)(lptr + 1) | _INT_LOWERCASE_Mask) == _TELNET_Mask1) {
				syntax = UriParser.TelnetUri;
				return true;
			}
			break;

		case if(nChars==8 && (*(lptr + 1) | _LOWERCASE_Mask) == _NETPIPE_Mask1) {
...
//#endif // PLATFORM_UNIX
			...
			DomainNameHelper.IsValid(pString, start, ref end, ref; dnsNotCanonical, StaticNotAny(flags, Flags.ImplicitFile)))
			{
				...
				if ((s_IdnScope != UriIdnScope.None)){
					...
					if (AllowIdnStatic(syntax, flags)){
						...
						string;
						idnValue = DomainNameHelper.UnicodeEquivalent(pString, start, end, ref;
						allAscii, ref;
						atLeastOneIdn;
					)
			...
			ref; bool; justNormalized, ref; string; newHost, ref; ParsingError; err)
		{
			...
			if (AllowIdnStatic(syntax, flags))
			{
				...
				string;
				idnValue = DomainNameHelper.IdnEquivalent(pString, start, end, ref;
				allAscii, ref;
				atLeastOneIdn;
			)
				...
				if (c <= '\x1F' || (c >= '\x7F' && c <= '\x9F'))
				{
					...
					if(m_iriParsing){
						...
						if (Char.IsHighSurrogate(c)){
							if ((i + 1) < end){
								bool; surrPair = false;
								valid = IriHelper.CheckIriUnicodeRange(c, str[i + 1], ref;
								surrPair, true;
							)
							}
						}
	...
	private unsafe; char[]; GetCanonicalPath(char[]; dest, ref; int; pos, UriFormat; formatAs)
		{
			...
			if (InFact(Flags.ShouldBeCompressed))
			{
				...
				dest = Compress(dest, (ushort)(pos + dosPathIdx), ref;
				end, m_Syntax;
			)
				if (dest[pos] == '\\')
					dest[pos] = '/';
	...
	private unsafe; static void UnescapeOnly(char* pch, int; start, ref; int; end, char; ch1, char; ch2, char; ch3) {
		...
		*(pnew-1) = ch;
			...
			ch = UriHelper.EscapedAscii((*pnew++ =*pch++;
		),
			(*pnew++ =;
		*
			pch++;
	...
	private static string; CombineUri(Uri; basePart, string; relativePart, UriFormat; uriFormat) {
			...
			if (basePart.HostType == Flags.IPv6HostType) {
				...
				if (basePart.IsImplicitFile) {
						...
						if (basePart.IsDosPath) {
							...
							path = Compress(path, 3, ref;
							length, basePart.Syntax;
						)
			...
			//compress the path
			path = Compress(path, basePart.SecuredPathIndex, ref;
			length, basePart.Syntax;
		)
	...
	protected static string; EscapeString(string; str) {
			...
			int; destStart = 0;
			char[];
			dest = UriHelper.EscapeString(str, 0, str.Length, null, ref;
			destStart, true, '?', '#', '%';
		)
			if ((object)dest == null)
			return str;
