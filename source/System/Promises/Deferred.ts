...
abstract class DeferredBase<T> implements IDeferredMinimal<T>
{
...
{
	onFulfill?:(value:T) => any | IPromiseMinimal<any>;
	onReject?:(error:any) => any | IPromiseMinimal<any>;
}

class Promise<T> implements IPromiseMinimal<T> {

	protected _promisees:PromiseHandler<T>[];
	...
	then<U>(
			onFulfill:(value:T)=>(IPromiseMinimal<U>|U),
			onReject:(error:any)=>(IPromiseMinimal<U>|U)):IPromiseMinimal<U>
	{
