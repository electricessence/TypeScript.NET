...
import LinkedList from "../../../../build/umd/dist/Collections/LinkedList";
...
describe('.addAfter & .addBefore', ()=>
{
	...
	it('should match expected initial count', ()=>
	{
		...
		assert.ok(areArraysEqual(parts, list1));
	});

	it('should match expected count after inserting before', ()=>
	{
		assert.equal(len2, count2);
		assert.ok(areArraysEqual(partsSpliced, list2), partsSpliced.join(',') + " != " + list2.join(','));
	});

	it('should match expected count after inserting after', ()=>
	{
		assert.equal(len2 + 1, count3);
		assert.ok(areArraysEqual(partsSpliced, list2), list3.join(','));
	});
