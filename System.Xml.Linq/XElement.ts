...
module System.Xml.Linq
{
	...
	export class XElement extends XContainer
	{
		...
		/*internal*/ //constructor(r: XmlReader, o?: LoadOptions);
		/*internal*/ //constructor();

		constructor(other: XStreamingElement);
		constructor(other: XElement);
		constructor(name: XName, ...content: any[]);
		constructor(other: any, ...content: any[])
		{
			super();

			if (other == null)
				if (name == null) throw 'ArgumentNullException("other")';

			var _ = this;

			if (other instanceof XName)
			{
				_._name = other;
			}
			else
			{
				if (content.length)
					throw 'Adding content is only allowed when specifying a name.';

				if (other instanceof XElement)
				{
					...
					if (a != null)
					{
						...
						{
							a = a.next;
							_._appendAttributeSkipNotify(new XAttribute(a));
						} while (a != other.lastAttr);
					}
				}
				else if (other instanceof XStreamingElement)
				{
					_._name = other.name;
					var c = other._content;
		...
		set name(value: XName)
		{
			...
			var _ = this;
			if (!value.equals(_._name))
			{
				_._notifyChange(_, XObjectChangeEventArgs.name,
					() => _._name = value);
			}
		}
		...
		{
			var lastAttr = this._lastAttr;
			return lastAttr != null ? lastAttr._next : null;
		}
		...
		get value(): string
		{
			...
			if (Types.isString(content)) return content;
