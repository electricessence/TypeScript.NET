...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "./Scheme", "./QueryParams", "../Exceptions/ArgumentException", "../Exceptions/ArgumentOutOfRangeException", "../Reflection/isString", "../Reflection/isNumber", "../Text/trim"], factory);
    }
    ...
    var Uri = /** @class */ (function () {
        ...
        Uri.from = function (uri, defaults) {
            var u = isString_1.default(uri)
                ? Uri.parse(uri) // Parsing a string should throw errors.  Null or undefined simply means empty.
                : uri;
            return new Uri(u && u.scheme || defaults && defaults.scheme, u && u.userInfo || defaults && defaults.userInfo, u && u.host || defaults && defaults.host, u && isNumber_1.default(u.port, true) ? u.port : defaults && defaults.port, u && u.path || defaults && defaults.path, u && u.query || defaults && defaults.query, u && u.fragment || defaults && defaults.fragment);
        };
    ...
    function getPort(port) {
        ...
        if (!port)
            return null;
        var p;
        if (isNumber_1.default(port)) {
            p = port;
            if (p >= 0 && isFinite(p))
                return p;
        }
        else if (isString_1.default(port) && (p = parseInt(port)) && !isNaN(p)) {
            return getPort(p);
        }
    ...
    function getAuthority(uri) {
        if (!uri.host) {
            ...
            if (isNumber_1.default(uri.port, true))
    ...
    function tryParse(url, out) {
        ...
        if (i != -1) {
            var scheme = trim_1.default(url.substring(0, i));
            var c = /:$/;
            if (!c.test(scheme))
                return new ArgumentException_1.default('url', 'Scheme was improperly formatted');
            scheme = trim_1.default(scheme.replace(c, EMPTY));
            try {
        ...
        i = url.indexOf(':');
        if (i != -1) {
            var port = parseInt(trim_1.default(url.substring(i + 1)));
            if (isNaN(port))
